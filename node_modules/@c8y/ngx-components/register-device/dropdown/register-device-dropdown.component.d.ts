import { Observable } from 'rxjs';
import { RegisterDeviceService } from '../register-device.service';
import { RegisterDeviceExtensionService } from './register-device-extension.service';
import * as i0 from "@angular/core";
export declare class RegisterDeviceDropdownComponent {
    private registerDeviceExtensionService;
    private registerDeviceService;
    single$: Observable<import("@c8y/ngx-components/register-device").RegisterDeviceItem[]>;
    bulk$: Observable<import("@c8y/ngx-components/register-device").RegisterDeviceItem[]>;
    limit$: Observable<boolean>;
    constructor(registerDeviceExtensionService: RegisterDeviceExtensionService, registerDeviceService: RegisterDeviceService);
    static ɵfac: i0.ɵɵFactoryDeclaration<RegisterDeviceDropdownComponent, never>;
    static ɵcmp: i0.ɵɵComponentDeclaration<RegisterDeviceDropdownComponent, "c8y-register-device-dropdown", never, {}, {}, never, never, false>;
}
