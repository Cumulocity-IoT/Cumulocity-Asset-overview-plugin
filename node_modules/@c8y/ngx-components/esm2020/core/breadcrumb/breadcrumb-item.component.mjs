import { Component, EnvironmentInjector, Injector, Input, ViewChild } from '@angular/core';
import * as i0 from "@angular/core";
export class BreadcrumbItemComponent {
    constructor(_injector) {
        this.injector = _injector;
    }
}
BreadcrumbItemComponent.ɵfac = i0.ɵɵngDeclareFactory({ minVersion: "12.0.0", version: "14.0.6", ngImport: i0, type: BreadcrumbItemComponent, deps: [{ token: i0.EnvironmentInjector }], target: i0.ɵɵFactoryTarget.Component });
BreadcrumbItemComponent.ɵcmp = i0.ɵɵngDeclareComponent({ minVersion: "14.0.0", version: "14.0.6", type: BreadcrumbItemComponent, selector: "c8y-breadcrumb-item", inputs: { icon: "icon", translate: "translate", label: "label", path: "path", injector: "injector" }, viewQueries: [{ propertyName: "template", first: true, predicate: ["template"], descendants: true }], ngImport: i0, template: "<ng-template #template>\n  <ng-content></ng-content>\n</ng-template>" });
i0.ɵɵngDeclareClassMetadata({ minVersion: "12.0.0", version: "14.0.6", ngImport: i0, type: BreadcrumbItemComponent, decorators: [{
            type: Component,
            args: [{ selector: 'c8y-breadcrumb-item', template: "<ng-template #template>\n  <ng-content></ng-content>\n</ng-template>" }]
        }], ctorParameters: function () { return [{ type: i0.EnvironmentInjector }]; }, propDecorators: { icon: [{
                type: Input
            }], translate: [{
                type: Input
            }], label: [{
                type: Input
            }], path: [{
                type: Input
            }], injector: [{
                type: Input
            }], template: [{
                type: ViewChild,
                args: ['template', { static: false }]
            }] } });
//# sourceMappingURL=data:application/json;base64,