...
const belowNodeVersion = '19.0.0';
function ngAdd(options) {
    return async (tree, _context) => {
        if (!(0, semver_1.satisfies)(process.version, `<${belowNodeVersion} >=${minNodeVersion}`)) {
            throw new schematics_1.SchematicsException(`You are running version ${process.version} of Node.js, which is not supported by c8y cli. Use a version >=14.0.0 and <19.0.0`);
        }
...
function angularJson(tree, options) {
    ...
    /**
     * Workaround, as dev-server breaks due to wrong angular.json schema.
     * Polyfills option breaks as it wants string and not an array.
     */
